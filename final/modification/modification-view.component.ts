...
export class ModificationViewComponent extends AViewComponent implements OnInit, OnDestroy, IViewComponent {
  ...
  // Display filter: 'all' | 'recent' | 'selected'
  public displayFilter: "all" | "recent" | "selected" = "all";
  ...
  /**
   * Apply display filter, search filter, and sorting.
  ...
  public applyFilterAndSort() {
    ...
    if (this.displayFilter === "recent") {
      ...
      result = result.filter((row) => {
        ...
        return (
          rowDate.getFullYear() === todayYear &&
          rowDate.getMonth() === todayMonth &&
          rowDate.getDate() === todayDay
        );
      });
    } else if (this.displayFilter === "selected") {
      // Placeholder: no additional filtering for now
    }
  ...
  /**
   * Handle display filter change (checkboxes behave like radio buttons).
   */
  public onDisplayFilterChange(filter: "all" | "recent" | "selected") {
